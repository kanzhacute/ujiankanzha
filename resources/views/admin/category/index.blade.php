                ...
                <tbody class="list">
                 @foreach ($category as $kategori)
                  <tr>
                    ...
                    <td>
                        <div class="d-flex">
                            <a class="btn btn-sm btn-icon-only text-primary" href="{{ route('category.show', ['category' => $kategori->id]) }}" role="button" data-toggle="tooltip" aria-haspopup="true" aria-expanded="false">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a class="btn btn-sm btn-icon-only text-warning" href="{{ route('category.edit', ['category' => $kategori->id]) }}" role="button" data-toggle="tooltip" aria-haspopup="true" aria-expanded="false">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('category.destroy', ['category' => $kategori->id])}}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-sm btn-icon-only text-light btn-danger" data-name="{{ $kategori->nama_kategori }}"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                    </td>
