              ...
              <table class="table align-items-center table-flush datatable">
                ...
                <tbody class="list">
                  ...
                  <tr>
                    ...
                    <td>
                        <div class="d-flex">
                            <a class="btn btn-sm btn-icon-only text-primary" href="{{ route('message.show', ['message' => $pesan->id]) }}" role="button" data-toggle="tooltip" aria-haspopup="true" aria-expanded="false">
                                <i class="fas fa-eye"></i>
                            </a>
                            <form action="{{ route('message.destroy', ['message' => $pesan->id])}}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-sm btn-icon-only text-light btn-danger" data-name="{{ $pesan->subject }}"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                    </td>
